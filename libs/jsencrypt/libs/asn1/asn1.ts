...
export class ASN1 {
    private constructor(stream:Stream, header:number, length:number, tag:ASN1Tag, sub:ASN1[]) {
    ...
    private static decodeLength(stream:Stream):number {
        let buf = stream.get();
        const len = buf & 0x7F;
        if (len == buf) {
            return len;
    ...
    private static decode_stream(str:Stream) {
                let stream:Stream;
        ...
        const getSub:() => ASN1[] = function () {
            ...
            if (len !== null) {
                ...
                try {
                    for (; ;) {
                        const s = ASN1.decode_stream(stream);
                        if (s.tag.isEOC()) {
                            break;
                        }
                        ret[ret.length] = s;
    ...
    public static decode(str:number[]) {
        let stream:Stream;
