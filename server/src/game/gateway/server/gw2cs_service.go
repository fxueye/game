...
func newGW2GSService(name string, ip string, port int) *GW2GSService {
	...
	serv.close = false
	serv.ClientService = tcp.NewClientService(ip, port, time.Second*30, serv.simpleRPC, serv, server.SessionConfig{4, 4})
	return serv
}
...
func (serv *GW2GSService) OnClose(*server.Session) {
	log.Infof("[%s] session closed", serv.name)
}
func (serv *GW2GSService) OnMessage(se *server.Session, p server.IPacket) bool {
	serv.msgTime = time.Now()
	defer func() {
		if err := recover(); err != nil {
			log.Error(err)
